# ONC Relational Findings - Open Nursing Core FHIR Implementation Guide (ONC-IG) v1.0.0

* [**Table of Contents**](toc.md)
* [**Artifacts Summary**](artifacts.md)
* **ONC Relational Findings**

## ValueSet: ONC Relational Findings 

| | |
| :--- | :--- |
| *Official URL*:https://opennursingcoreig.com/ValueSet/onc-relational-findings-vs | *Version*:1.0.0 |
| Active as of 2026-07-11 | *Computable Name*:ONCRelationalFindingsVS |

 
The relational-care findings that the onc-to-nanda ConceptMap maps to formal nursing diagnoses. 

 **References** 

This value set is not used here; it may be used elsewhere (e.g. specifications and/or implementations that use this content)

### Logical Definition (CLD)

 

### Expansion

-------

 Explanation of the columns that may appear on this page: 

| | |
| :--- | :--- |
| Level | A few code lists that FHIR defines are hierarchical - each code is assigned a level. In this scheme, some codes are under other codes, and imply that the code they are under also applies |
| System | The source of the definition of the code (when the value set draws in codes defined elsewhere) |
| Code | The code (used as the code in the resource instance) |
| Display | The display (used in the*display*element of a[Coding](http://hl7.org/fhir/R4/datatypes.html#Coding)). If there is no display, implementers should not simply display the code, but map the concept into their application |
| Definition | An explanation of the meaning of the concept |
| Comments | Additional notes about how to use the code |



## Resource Content

```json
{
  "resourceType" : "ValueSet",
  "id" : "onc-relational-findings-vs",
  "url" : "https://opennursingcoreig.com/ValueSet/onc-relational-findings-vs",
  "version" : "1.0.0",
  "name" : "ONCRelationalFindingsVS",
  "title" : "ONC Relational Findings",
  "status" : "active",
  "experimental" : false,
  "date" : "2026-07-11T14:32:04+00:00",
  "publisher" : "The Open Nursing Community",
  "description" : "The relational-care findings that the onc-to-nanda ConceptMap maps to formal nursing diagnoses.",
  "compose" : {
    "include" : [{
      "system" : "https://opennursingcoreig.com/CodeSystem/onc-observation-codes",
      "concept" : [{
        "code" : "patient-story",
        "display" : "Patient Story"
      },
      {
        "code" : "relational-engagement",
        "display" : "Relational Engagement Score"
      },
      {
        "code" : "mst-score",
        "display" : "Monk Skin Tone Score"
      }]
    }]
  }
}

```
